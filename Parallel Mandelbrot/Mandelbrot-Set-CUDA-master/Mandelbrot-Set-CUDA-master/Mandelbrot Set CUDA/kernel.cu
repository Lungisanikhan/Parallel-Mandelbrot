...
__global__ void mandelbrotKernel(Mandelbrot mandelbrot, double* cr, double* ci) {
	...
	*/
	...
	while (i < maxIterations && zr * zr + zi * zi < 4.0) {
		...
		//Calculate the z imaginari value using the c imaginary value
	...
	//comment out below to see colour
	//r = (i % 256); //change for more interesting values!
	//g = (i % 256); //change for more interesting values!
	//b = (i % 256); //change for more interesting values!
	///
